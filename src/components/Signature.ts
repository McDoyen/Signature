...
export class SignatureCanvas extends Component<SignatureProps, Signaturestate> {
	...
	private points: any[] = [];
	private lastWidth = (this.props.minWidth + this.props.maxWidth) / 2;
	private timer = 0;
	...
	render() {
		return createElement("div", {
			className: classNames(
				this.state.signature_set ? "widget-Signature signature-set" : "widget-Signature signature-unset",
			...
			createElement("canvas", {
				...
				style: {
					...
					display: this.state.signature_set ? "none" : "block"
				},
			...
			createElement("img", {
				...
				src: this.state.signature_set ? this.image.src : "",
				style: {
					border: this.props.gridBorder + "px solid",
					display: this.state.signature_set ? "block" : "none",
					opacity: 0.5
				},
				width: this.props.width
			}),
	...
	componentWillReceiveProps(newProps: SignatureProps) {
		this.setState({
			imageSource: newProps.imageSource
		});
	}
	...
	private eventResetClicked() {
		if (this.props.resetMxObject) {
			this.props.resetMxObject();
		}
		this.resetCanvas();
		this.hideImage();
	...
	private endCurve(e: PointerEvent) {
		...
		if (this.props.finalizeSignature) {
			this.timer = setTimeout(this.props.finalizeSignature(this.canvas), this.props.timeOut);
